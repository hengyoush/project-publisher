 ...
 * copy到packageRoot下的lib目录,将application.properties和key.properties也拷贝到 指定目录的lib下。
 */
public class DefaultLibHandler implements DirHandler {
    ...
    public void handle(Project project) {
        ...
        String targetPath = Util.getSrcTargetPath(project);
        FilePath targetFilePath = Util.getFilePathFromPathStr(targetPath);
        FilePath libFilePath = new FilePath(new File(libPath));
        try {
            copyJarsToLib(project, libFilePath);
            copyJarsToLib(project, targetFilePath);
    ...
    private void copyJarsToLib(Project project, FilePath targetFilePath) throws IOException, InterruptedException {
        FilePath[] targetJars = targetFilePath.list("*jar");
        for (FilePath srcLib : targetJars) {
            String targetPathStr = Util.getTargetDirStr(project, DirType.LIB) + File.separator + srcLib.getName();
